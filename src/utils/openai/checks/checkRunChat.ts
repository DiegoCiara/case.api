...
export async function checkRunChat(openai: OpenAI, thread: Thread, runId: string, workspace: Workspace, assistant: Assistant): Promise<any> {
  return await new Promise((resolve, reject) => {
    ...
    const verify = async (): Promise<void> => {
      ...
      if (runStatus.status === 'completed') {
        ...
        (await ioSocket).emit(`proccessingChat-error:${thread.id}`, '')
        ...
        (await ioSocket).emit(`proccessingChat:${thread.id}`, '')
