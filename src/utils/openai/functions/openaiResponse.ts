...
import { checkRunChat, checkRunStatus, getActiveRun } from '../checks/checkRunChat';
...
export async function openaiResponse(
  ...
  try {
    ...
    if (activeRun) {
      const messages = await checkRunChat(openai, thread, activeRun.id, workspace, assistant);
      return {
    ...
    return new Promise(async(resolve, reject) => {
        try {
          ...
          const run = await openai.beta.threads.runs.create(openAIThreadId, {
            ...
            instructions: instructions || openAIAssistant.instructions,
          });

          const messages = await checkRunChat(openai, thread, run.id, workspace, assistant);
          if (!messages) {
            resolve({
              openAIThreadId: openAIThreadId,
