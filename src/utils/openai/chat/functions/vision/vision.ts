...
import PlaygroundVision from '@entities/PlaygroundVision';
 ...
 */
export const vision = async (openai: OpenAI, imageS3Location: string, workspace: Workspace, threadId: string, type: string) => {
  try {
    ...
    if (type === 'playground') {
      ...
      const vision = await PlaygroundVision.create({
        width: result.width,
